...
def draw_survival_curves_and_histograms(simOutputs_notherapy, simOutputs_anticoag):
    ...
    survival_curves = [
        simOutputs_notherapy.get_survival_curve(),
    ...
    set_of_survival_times = [
        simOutputs_notherapy.get_survival_times(),
    ...
    Figs.graph_histograms(
        ...
        y_label='Counts',
        bin_width=1,
        legend=['No Therapy', 'Anticoag Therapy'],
        transparency=0.6
    )


def print_comparative_outcomes(simOutputs_notherapy, simOutputs_anticoag):
    ...
    # increase in survival time under combination therapy with respect to mono therapy
        increase_survival_time=Stat.DifferenceStatPaired(
            ...
            y_ref=simOutputs_notherapy.get_survival_times()) #Independent because we cannot make these outcomes to be the same minus the disease outcome
    else:
        increase_survival_time=Stat.DifferenceStatIndp(
            name='Increase in survival time',
            x=simOutputs_anticoag.get_survival_times(),
            y_ref=simOutputs_notherapy.get_survival_times()
        )
    ...
    #CI because it was meant for a cohort with many patients over a long period of time (rather than PI)
    # estimate and CI
    ...
    if Settings.PSA_ON:
        increase_discounted_cost = Stat.DifferenceStatPaired(
            ...
            y_ref=simOutputs_notherapy.get_costs())
    else:
    ...
    if Settings.PSA_ON:
        increase_discounted_utility = Stat.DifferenceStatPaired(
            ...
            y_ref=simOutputs_notherapy.get_utilities())
    else:
...
def report_CEA_CBA(simOutputs_notherapy, simOutputs_anticoag):
    ...
    no_therapy_strategy = Econ.Strategy(
        name = 'No Therapy Strategy',
        cost_obs=simOutputs_notherapy.get_costs(),
        effect_obs=simOutputs_notherapy.get_utilities()
    )
    anticoag_therapy_strategy = Econ.Strategy(
        name='Anticoag Therapy Strategy',
